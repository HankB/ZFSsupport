...
sub main {
	...
	if ( defined $MyZFS::trial ) {
		print( "found ", scalar(@snapshots), " snapshots\n\t",join( "\n\t", @snapshots ), "\n" );
	}

	my @deletableSnaps = MyZFS->getDeletableSnaps(@snapshots);
	if ( defined $MyZFS::trial ) {
		print( "found ", scalar(@deletableSnaps), " deletable snapshots\n\t",join( "\n\t", @deletableSnaps ), "\n" );
	}

	my @snapsToDelete  = MyZFS->getSnapsToDelete( \@deletableSnaps,$MyZFS::reserveCount );
	if ( defined $MyZFS::trial ) {
		print( "found ", scalar(@snapsToDelete), " snapshots to destroy\n\t",join( "\n\t", @snapsToDelete ), "\n" );
	} else {
        MyZFS->destroySnapshots(@snapsToDelete);
    }

	my @dumpsToDelete = MyZFS->findDeletableDumps($MyZFS::dumpDirectory, \@snapsToDelete);

	if ( defined $MyZFS::trial ) {
		print( "found ", scalar(@dumpsToDelete), " dumps to destroy\n\t",join( "\n\t", @dumpsToDelete ), "\n" );
	} else {
